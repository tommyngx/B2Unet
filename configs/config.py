...
class Config:
    def __init__(self, model_name="unetplusplus"):
        ...
        # Data
        self.DATA_DIR = "/path/to/dataset"  # Update with the path to your dataset directory
        self.IMAGES_DIR = f"{self.DATA_DIR}/images"
        self.MASKS_DIR = f"{self.DATA_DIR}/masks"
        self.TRAIN_CSV_PATH = f"{self.DATA_DIR}/train.csv"
        self.VALID_CSV_PATH = f"{self.DATA_DIR}/valid.csv"  # Assuming you have a separate validation split
        self.TEST_CSV_PATH = f"{self.DATA_DIR}/test.csv"

        # Model Saving
        self.MODEL_SAVE_PATH = f"/path/to/save/{self.PROJECT_NAME}_model.pth"
